...
- **A fresh safety read preserves ADR 0001.** Perch checks dirtiness immediately before moving the directory. A clean reading may take the fast path. A dirty or unreadable result may take it only when the License covers discarding files. Without that License, uncertainty goes through ordinary unforced `git worktree remove`, leaving Git's guard in charge. A missing directory also stays on the ordinary path because there is nothing to stage.
