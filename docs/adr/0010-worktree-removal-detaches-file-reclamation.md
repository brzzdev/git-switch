...
- **Deregistration gates deletion.** The background worker does not start until Git has removed and verified the worktree registration. A refusal restores the staged directory to its original path. A process error attempts the same restoration before it returns.
