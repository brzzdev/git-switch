...
But the mode is not a question the user should answer. It is a property of the thing they are picking, and the tool already knows it. So all three verbs draw **one list of every branch**, in one rendering, with the worktree-backed ones annotated with their path. What the verb changes is what selecting a row *does* — and, in the one case where that is nothing, whether the row can be selected at all.

## Consequences

- **The *Worktrees* section is gone.** A branch appears once, under the heading it belongs to, carrying its worktree path as an *Annotation*. Two sections listing the same branch under different rules is the mode question again, asked in the layout instead of in a prompt.
...
- **The list is a pure function.** Given the branches and the worktrees, it produces the rows, their *Annotation*s, and the disabled set with no repo on disk — which is where the rule above is tested, rather than through a terminal.
